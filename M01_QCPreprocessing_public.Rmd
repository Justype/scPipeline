...
parameter.list <- list(
  ...
  # input parameters
  input.object = NULL,       
  input.cell_ranger = "filtered_gene_bc_matrices/hg19/", 
  input.matrix = NULL,

  # output parameters
  save.flag = T,              
  output.object.directory = "Preprocessed_Datasets/",
  output.object.file = "pbmc.Rdata",
  ...
  scale.method = "sct",                      # option: "sct" (recommended), "null_residuals" or "nfs"
  ...
  species.include = "Hs",                    # required; options: Hs, Mm
...
dir.create(parameter.list$output.object.directory, recursive = T)
```
...
for (i in 1:length(input_data)){
  if (input.type[i] == 0){
    ...
    if (grepl(".rds", input_data[i])){
      ...
      names(output.list) <- c("object","genes")
  ...
  if (all((unique(so.list[[i]]@meta.data[["Barcode"]]) == "unspecified") )){
    so.list[[i]]@meta.data[["Barcode"]] <- paste("T", i, sep = "")
  } else if (all(is.na(unique(so.list[[i]]@meta.data[["Barcode"]])))){
...
so <- tryCatch({
 scNormScale(object = so, 
              method = parameter.list$normalization_method_ps, 
  ...
  if (!conserve.memory){
    ...
    if (!is.success){
      parameter.list$normalization_method_ps <- "NFS"  # "NFS" 
      so <- scNormScale(object = so, 
                        method = parameter.list$normalization_method_ps, 
                        vars2regress = parameter.list$vars2regress,
                        n.workers = parameter.list$nworkers,  
...
if (nassay == "RNA"){
 parameter.list$normalization_method_ps = "NFS"
 parameter.list$scale.method <- "nfs"
} else if (nassay == "SCT"){
  parameter.list$normalization_method_ps = "SCT"
...
# transform so to SingleCellExperiment

so.sce = as.SingleCellExperiment(so)

so.sce <- schex::make_hexbin(so.sce, nbins = 80, dimension_reduction = "UMAP")
plt.umap_umi <- schex::plot_hexbin_meta(so.sce, col="nCount_RNA", 
                           action="mean", xlab="UMAP1", ylab="UMAP2") + 
  labs(title = "Transcript counts (n/cell)", subtitle = "Post-Filtering")
plt.umap_features <- schex::plot_hexbin_meta(so.sce, col="nFeature_RNA", 
                           action="mean", xlab="UMAP1", ylab="UMAP2") + 
  labs(title = "Gene counts (n/cell)", subtitle = "Post-Filtering")
plt.umap_mito <- schex::plot_hexbin_meta(so.sce, col="percent.mt", 
                           action="mean", xlab="UMAP1", ylab="UMAP2") + 
...
plt.umap_umr <- schex::plot_hexbin_meta(so.sce, col="unmatched.rate", 
                           action="mean", xlab="UMAP1", ylab="UMAP2") + 
...
if (parameter.list$normalization_method_ps == "SCT"){
...
if (parameter.list$normalization_method_ps == "NFS"){
  norm_method <- "normalize, find features, scale"
} else if (parameter.list$normalization_method_ps == "SCT"){
  norm_method <- "SCT"
}
